...
INPUT_FOLDER = r"C:\Users\daniel\Desktop\LLM_FINAL\data\analisados_manualmente_few_shot"
OUTPUT_FOLDER = r"C:\Users\daniel\Desktop\LLM_FINAL\f1_score\f1"
...
def plot_confusion(cm: np.ndarray, labels, title: str, outpath_png: str, outpath_svg: str, normalize: bool = False):
    ...
    im = ax.imshow(cm_to_plot, aspect='auto', cmap=CMAP_CONFUSION, vmin=0)
...
def bars_metrics(df_metrics: pd.DataFrame, outpath_png: str, outpath_svg: str, sort_by: str = "f1_macro"):
    ...
    bars1 = ax.bar(x - width, df_plot["accuracy"], width, label="Accuracy",
                   color=PALETTE["accuracy"], edgecolor=BAR_EDGECOLOR, linewidth=BAR_LINEWIDTH)
    bars2 = ax.bar(x,          df_plot["f1_macro"], width, label="F1 Macro",
                   color=PALETTE["f1_macro"], edgecolor=BAR_EDGECOLOR, linewidth=BAR_LINEWIDTH)
    bars3 = ax.bar(x + width,  df_plot["f1_micro"], width, label="F1 Micro",
                   color=PALETTE["f1_micro"], edgecolor=BAR_EDGECOLOR, linewidth=BAR_LINEWIDTH)
    ax.set_xticks(x)
...
def heatmap_per_class_f1(df_reports: pd.DataFrame, outpath_png: str, outpath_svg: str):
    ...
    im = ax.imshow(data, aspect='auto', vmin=0, vmax=1, cmap=CMAP_HEATMAP)
